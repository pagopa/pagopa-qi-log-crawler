 ...
 *
 */
#[TestDox('[T00033] Valuta la corretta lavorazione di un workflow di pagamento MOD3 NEW con 1 transfer e SPO multiple')]
class T00033_WorkflowPaymentMod3New1TransferMultipleSPOOK extends TestCase
{
    ...
    public function testTransaction()
    {
        ...
        $this->assertEquals('2024-03-10 14:05:00.201', $transaction->getColumnValue('inserted_timestamp'));
        $this->assertEquals('01000000000000033', $transaction->getColumnValue('iuv'));
        $this->assertEquals('77777777777', $transaction->getColumnValue('pa_emittente'));
        $this->assertEquals('301000000000000033', $transaction->getColumnValue('notice_id'));
        $this->assertEquals('t0000000000000000000000000000033', $transaction->getColumnValue('token_ccp'));
    ...
    public function testTransactionDetails()
    {
        ...
        $this->assertEquals('245.00', $details->getColumnValue('amount_transfer'));
    ...
    {
        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 54);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));

        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 55);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));

        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 56);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));

        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 57);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));

        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 58);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));

        $event = self::$db->getReEvent(new \DateTime('2024-03-10'), 59);
        $this->assertEquals('LOADED', $event->getColumnValue('state'));
    }
}
