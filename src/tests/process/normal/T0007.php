...
#[TestDox('[CRAWLER] Lavorazione di una activatePaymentNotice REQ prelievo del dominio dal payload')]
class T0007 extends TestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $result = Capsule::table('transaction_2024')
            ->where('iuv', '=', '01000000000000007')
            ->where('token_ccp', '=', 't0000000000000000000000000000007')
            ->get();
    ...
    public function testCorrectDateEvent()
    {
        ...
        $this->assertEquals('2024-03-10 09:10:17.232',self::$transaction->getColumnValue('inserted_timestamp'));
        $this->assertEquals('01000000000000007',self::$transaction->getColumnValue('iuv'));
        $this->assertEquals('77777777777',self::$transaction->getColumnValue('pa_emittente'));
        $this->assertEquals('301000000000000007',self::$transaction->getColumnValue('notice_id'));
        $this->assertNull(self::$transaction->getColumnValue('id_carrello'));
        $this->assertEquals('t0000000000000000000000000000007', self::$transaction->getColumnValue('token_ccp'));
    ...
    public function testCorrectWorkflowDateEvent()
    {
        ...
        $this->assertEquals('2024-03-10 09:10:17.232', self::$workflow->getColumnValue('event_timestamp'));
        $this->assertEquals(1, self::$workflow->getColumnValue('fk_tipoevento'));
        $this->assertEquals('T000007', self::$workflow->getColumnValue('event_id'));
    }
