...
#[TestDox('[CRAWLER] Lavorazione di una activatePaymentNotice REQ prelievo del PSP dal payload')]
class T0002 extends TestCase
{
    ...
    public function testCorrectInfoTransaction()
    {
        $this->assertEquals('2024-03-10',self::$transaction->getColumnValue('date_event'));
        $this->assertEquals('2024-03-10 09:10:11.232',self::$transaction->getColumnValue('inserted_timestamp'));
        $this->assertEquals('01000000000000002',self::$transaction->getColumnValue('iuv'));
        $this->assertEquals('77777777777',self::$transaction->getColumnValue('pa_emittente'));
        $this->assertEquals('301000000000000002',self::$transaction->getColumnValue('notice_id'));
        $this->assertNull(self::$transaction->getColumnValue('id_carrello'));
        $this->assertEquals('t0000000000000000000000000000002',self::$transaction->getColumnValue('token_ccp'));
        ...
        $this->assertEquals('PSP_03',self::$transaction->getColumnValue('id_psp'));
    ...
    public function testCorrectWorkflow()
    {
        ...
        $this->assertEquals('2024-03-10 09:10:11.232', self::$workflow->getColumnValue('event_timestamp'));
        $this->assertEquals(1, self::$workflow->getColumnValue('fk_tipoevento'));
        $this->assertEquals('T000002', self::$workflow->getColumnValue('event_id'));
    }
