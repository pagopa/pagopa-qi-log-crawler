...
#[TestDox('[CRAWLER] Lavorazione di una activatePaymentNotice REQ prelievo del Canale dal payload')]
class T0003 extends TestCase
{
    ...
    public function testCorrectDateEvent()
    {
        ...
        $this->assertEquals('2024-03-10 09:10:12.232',self::$transaction->getColumnValue('inserted_timestamp'));
        $this->assertEquals('01000000000000003',self::$transaction->getColumnValue('iuv'));
        $this->assertEquals('77777777777',self::$transaction->getColumnValue('pa_emittente'));
        $this->assertEquals('301000000000000003',self::$transaction->getColumnValue('notice_id'));
        $this->assertNull(self::$transaction->getColumnValue('id_carrello'));
        $this->assertEquals('t0000000000000000000000000000003',self::$transaction->getColumnValue('token_ccp'));
        $this->assertEquals('PSP_02',self::$transaction->getColumnValue('id_psp'));
        $this->assertEquals('77777777777_01',self::$transaction->getColumnValue('stazione'));
        $this->assertEquals('88888888888_02',self::$transaction->getColumnValue('canale'));
    ...
    public function testCorrectWorkflowDateEvent()
    {
        ...
        $this->assertEquals('2024-03-10 09:10:12.232', self::$workflow->getColumnValue('event_timestamp'));
        $this->assertEquals(1, self::$workflow->getColumnValue('fk_tipoevento'));
        $this->assertEquals('T000003', self::$workflow->getColumnValue('event_id'));
        $this->assertEquals('77777777777_01',self::$workflow->getColumnValue('stazione'));
        $this->assertEquals('88888888888_02',self::$workflow->getColumnValue('canale'));
    }
