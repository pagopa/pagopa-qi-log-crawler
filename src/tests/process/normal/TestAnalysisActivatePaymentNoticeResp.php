...
#[TestDox('[CRAWLER] Lavorazione eventi di activatePaymentNotice Response')]
class TestAnalysisActivatePaymentNoticeResp extends TestCase
{
    ...
    #[TestDox('[activatePaymentNotice RESP] Aggiornamento prezzo su tabella Transaction')]
    public function testUpdateTotalAmount()
    {
    ...
    #[TestDox('[activatePaymentNotice RESP] Inserimento evento nella transaction_workflow')]
    public function testInsertEventIntoWorkflow()
    {
        $result_transaction = Capsule::table('transaction_2024')
            ->where('iuv', '=', '01000000000000001')
            ->where('token_ccp' , '=', 't0000000000000000000000000000001')
        ...
        $result_transaction = Capsule::table('transaction_events_2024')
            ->where('fk_payment', '=', $id)
            ->where('event_id', '=', 'test_000008')
            ->get();

        $transaction_workflow = new Workflow(new \DateTime('2024-03-10'), (array) $result_transaction->get(0));
        $this->assertEquals(2, $transaction_workflow->getColumnValue('fk_tipoevento'));
        $this->assertEquals('test_000008', $transaction_workflow->getColumnValue('event_id'));

    }


    #[TestDox('[activatePaymentNotice RESP] Inserimento dei transfer nella transaction_details')]
