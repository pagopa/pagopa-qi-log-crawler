...
class AbstractXmlPayload extends AbstractPayload
{
    ...
    /**
     * Constante che definisce l'XPATH dove trovare il CCP
     */
    const  XPATH_CCP               = null;

    /**
     * Constante che definisce l'XPATH dove trovare il TOKEN
     */
    const  XPATH_TOKEN               = null;
    ...
    {
        $p = $index + 1;
        if (($p > $this->getPaymentsCount()) || (static::XPATH_CCP == null))
        {
            return null;
        }

        $render_xpath = vsprintf(static::XPATH_CCP, [$p]);
        return $this->getElement($render_xpath);
    }
    ...
    {
        $p = $index + 1;
        if (($p > $this->getPaymentsCount()) || (static::XPATH_TOKEN == null))
        {
            return null;
        }
        $render_xpath = vsprintf(static::XPATH_TOKEN, [$p]);
        return $this->getElement($render_xpath);
    }
